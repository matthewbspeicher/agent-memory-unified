#!/usr/bin/env python3
"""Force Railway services to use NIXPACKS builder"""
...
def update_service(service_name, config):
    ...
    variables = {
        ...
        "input": {
            ...
            "dockerfilePath": None,  # Clear Dockerfile path
        }
    }

    token = get_token()
    print(f"🔧 Forcing {service_name} to use {config['builder']}")
...
def main():
    print("🔧 Forcing Railway services to use NIXPACKS\n")

    for name, config in SERVICES.items():
        update_service(name, config)

    print("✅ Builder updates complete")
