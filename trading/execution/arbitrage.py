...
class ArbCoordinator:
    ...
    async def _log_journal_entry(self, trade: ArbTrade):
        ...
        await self._journal_manager.log_trade_entry(trade.id, decision, execution, tags=["arbitrage", "track16"])
    ...
    async def execute_arbitrage(self, trade: ArbTrade) -> bool:
        ...
        """
        logger.info(f"ArbCoordinator: Starting arbitrage {trade.id} for {trade.symbol_a} ↔ {trade.symbol_b}")
        ...
        try:
            ...
            if self._meta_agent:
                ...
                if max_tox > self._config.arb_toxicity_threshold:
                    logger.warning(f"ArbCoordinator: Aborting {trade.id} - Toxicity too high: {max_tox:.2f}")
                    trade.state = ArbState.FAILED
                    trade.error_message = f"Toxicity too high: {max_tox:.2f}"
                    await self._publish_state(trade)
                    await self._log_journal_exit_wrapper(trade, pnl=0.0, reason="Aborted: Toxicity")
                    return False

            # 2. Pre-flight slippage check
            if not await self._check_preflight_slippage(trade):
                logger.warning(f"ArbCoordinator: Aborting {trade.id} due to excessive estimated slippage.")
                trade.state = ArbState.FAILED
                trade.error_message = "Excessive estimated slippage"
                await self._publish_state(trade)
                await self._log_journal_exit_wrapper(trade, pnl=0.0, reason="Aborted: Slippage")
                return False
            ...
            if success:
                ...
                await self._publish_state(trade)
                logger.info(f"ArbCoordinator: Arbitrage {trade.id} completed successfully.")
                expected_pnl_usd = float(trade.expected_profit_bps) / 100.0
                await self._log_journal_exit_wrapper(trade, pnl=expected_pnl_usd, reason="Arb executed successfully")
                return True
            else:
            ...
            logger.exception(f"ArbCoordinator: Fatal error executing arbitrage {trade.id}")
            trade.state = ArbState.FAILED
            trade.error_message = str(e)
            await self._store.update_trade_state(trade.id, trade.state, trade.error_message)
            await self._publish_state(trade)
            await self._log_journal_exit_wrapper(trade, pnl=0.0, reason=f"Fatal error: {e}")
            return False
    ...
    async def _execute_legs_concurrently(self, trade: ArbTrade) -> bool:
        ...
        if success_a and not success_b:
            logger.warning(f"ArbCoordinator: Leg B failed, unwinding Leg A for {trade.id}")
            ...
            await self._log_journal_exit_wrapper(trade, pnl=-1.0, reason="Unwound Leg A after Leg B failure")
        elif success_b and not success_a:
            logger.warning(f"ArbCoordinator: Leg A failed, unwinding Leg B for {trade.id}")
            trade.state = ArbState.UNWINDING
            await self._store.update_trade_state(trade.id, trade.state, "Leg A failed")
            await self._publish_state(trade)
            await self._unwind_leg(trade, "leg_b")
            await self._log_journal_exit_wrapper(trade, pnl=-1.0, reason="Unwound Leg B after Leg A failure")
        else:
            ...
            await self._store.update_trade_state(trade.id, trade.state, trade.error_message)
            await self._publish_state(trade)
            await self._log_journal_exit_wrapper(trade, pnl=0.0, reason="Both legs failed")
        ...
        try:
            ...
            if poly_broker:
                ...
                if ds and hasattr(ds, "get_live_quote"):
                    ...
                    if price is None:
                        logger.warning(f"Slippage check: No live quote for {trade.symbol_b}")
                        return False
                else:
                    ...
                    try:
                        ...
                        if quote is None:
                            logger.warning(f"Slippage check: No quote for {trade.symbol_b}")
                            return False
        ...
        try:
            async with asyncio.timeout(self._config.order_timeout):
                ...
                if order_result.status == OrderStatus.FILLED:
                    ...
                    leg.fill_price = order_result.avg_fill_price or leg.order.limit_price if hasattr(leg.order, 'limit_price') else Decimal("0")
                    leg.fill_quantity = order_result.filled_quantity or leg.order.quantity
                    leg.status = "submitted"
                
                await self._store.update_leg_atomic(trade.id, leg_name, leg)
                return order_result.status in (OrderStatus.FILLED, OrderStatus.SUBMITTED)
    ...
    async def _unwind_leg(self, trade: ArbTrade, leg_name: str):
        ...
        logger.warning(f"ArbCoordinator: Unwinding {leg_name} ({leg.fill_quantity} @ {leg.fill_price})")
        
        unwind_side = OrderSide.SELL if leg.order.side == OrderSide.BUY else OrderSide.BUY
        ...
        try:
            async with asyncio.timeout(self._config.order_timeout):
                await broker.orders.place_order(unwind_order.account_id or "", unwind_order)
            ...
            logger.error(f"ArbCoordinator: CRITICAL - Unwind timed out for {leg_name} on {trade.id}")
            await self._store.update_trade_state(trade.id, ArbState.FAILED, f"Unwind timed out for {leg_name}")
        except Exception as e:
            logger.error(f"ArbCoordinator: CRITICAL - Failed to unwind {leg_name} for {trade.id}: {e}")
            await self._store.update_trade_state(trade.id, ArbState.FAILED, f"Unwind failed: {e}")
