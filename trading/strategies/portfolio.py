    ...
    async def scan(self, data: DataBus) -> list[Opportunity]:
        ...
        for t in trades:
            order_res = (
                ...
                if isinstance(t["order_result"], str)
                else t["order_result"]
            )
            ...
            if sym and sym not in last_trade_dates:
                ...
                if created_str:
                    try:
                        last_trade_dates[sym] = datetime.fromisoformat(
        ...
        for pos in positions:
            ...
            if reasons:
                opp = Opportunity(
                    ...
                    data={
                        "pnl_pct": pct_return,
                        "stale": bool(last_date and days_held >= stale_days),
                    },
                    timestamp=now,
                )
    ...
    async def scan(self, data: DataBus) -> list[Opportunity]:
            ...
            if sym:
                ...
                if created_str:
                    try:
                        ...
                        if (
                            sym not in recent_trade_dates
                            or dt > recent_trade_dates[sym]
                        ):
        ...
        for pos in positions:
            ...
            if last_date and (now - last_date).days <= wash_sale_days:
                logger.info(
                    "Skipping tax loss harvest for %s due to wash sale rule (traded %d days ago)",
                    pos.symbol.ticker,
                    (now - last_date).days,
                )
                continue
