...
class OptionsHedgingAgent(StructuredAgent):
    ...
    async def scan(self, data: DataBus) -> list[Opportunity]:
        ...
        if (
            self.universe
            and "PORTFOLIO" not in self.universe
            and self.universe != "ALL"
        ):
        ...
        Decimal(str(self.parameters.get("target_delta", 0.16)))
        ...
        for pos in positions:
            ...
            try:
                ...
                logger.debug(
                    "Could not fetch options chain for %s: %s", pos.symbol.ticker, e
                )
                continue
            ...
            valid_expiries = [
                d for d in chain.expirations if dte_min <= (d - now).days <= dte_max
            ]
            ...
            if strategy_type == "collar":
                put_strike = find_closest_strike(
                    strikes, current_price * Decimal("0.90")
                )
                call_strike = find_closest_strike(
                    strikes, current_price * Decimal("1.10")
                )
                ...
                put_short = find_closest_strike(
                    strikes, current_price * Decimal("0.90")
                )
                call_short = find_closest_strike(
                    strikes, current_price * Decimal("1.10")
                )
                call_long = find_closest_strike(
                    strikes, current_price * Decimal("1.15")
                )
                ...
                suggested_trade = build_iron_butterfly(
                    "PAPER", pos.symbol, Decimal("1"), expiry, atm, put_long, call_long
                )
                reasoning = (
                ...
                put_strike = find_closest_strike(
                    strikes, current_price * Decimal("0.95")
                )
