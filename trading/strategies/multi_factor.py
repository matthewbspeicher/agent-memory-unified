    ...
    async def scan(self, data: DataBus) -> list[Opportunity]:
        ...
        for symbol in symbols:
            try:
                ...
                macd_diff = float(macd_result.macd_line) - float(
                ...
                avg_volume = (
                    sum(b.volume for b in lookback_bars) / len(lookback_bars)
                    if lookback_bars
                    else 1
                )
                current_volume = current_bar.volume
                volume_factor = (
                    min((current_volume / avg_volume - 1.0), 1.0)
                    if avg_volume > 0
                    else 0.0
                )
                ...
                logger.warning(
                    "MultiFactorAgent scan failed for %s: %s", symbol.ticker, e
                )
