...
class KalshiConnection(BrokerConnection):
    ...
    async def connect(self) -> None:
        try:
            ...
            logger.warning(
                "Kalshi: connection check failed (read-only public data still available): %s",
                exc,
            )
...
class KalshiAccount(AccountProvider):
    ...
    async def get_positions(self, account_id: str) -> list[Position]:
        ...
        for p in raw:
            ...
            Decimal(str(p.get("fees_paid", 0)))  # best proxy available
        ...
        if self._ws_task is None:
            self._ws_task = asyncio.create_task(
                self._client.ws_connect(
                    channels=["ticker"], callback=self._on_ws_message
                )
            )
...
class KalshiOrderManager(OrderManager):
    ...
    async def place_order(self, account_id: str, order: OrderBase) -> OrderResult:
        ...
        try:
            ...
            status = (
                OrderStatus.SUBMITTED if "rest" in status_str else OrderStatus.FILLED
            )
    ...
    async def modify_order(self, order_id: str, changes: dict) -> OrderResult:
        raise NotImplementedError(
            "Kalshi does not support order modification; cancel and re-submit"
        )
