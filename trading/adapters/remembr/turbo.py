    ...
    def search(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            labels, distances = self.index.knn_query(
                [vector], k=min(k, self.index.get_current_count())
            )
    ...
    def load(self, path: str, max_elements: int = 100000):
        ...
        with self._lock:
            self.index.load_index(
                os.path.join(path, "turboquant.bin"), max_elements=max_elements
            )
            ...
            self.metadata_cache = {
                int(k): v for k, v in state.get("metadata_cache", {}).items()
            }
