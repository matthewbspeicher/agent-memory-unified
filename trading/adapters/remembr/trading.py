...
class TradingJournal:
    ...
    def execute_trade(
        ...
        memory_resp = self.client.store(
            value=reasoning, metadata=metadata, tags=["trade", ticker]
        )
        ...
        if not memory_id:
            raise Exception(
                "API Error: Failed to retrieve memory_id from store() response"
            )
    ...
    def close_trade(
        ...
        metadata = {"type": "trade_outcome", "parent_trade_id": parent_trade_id}

        memory_resp = self.client.store(
            value=reasoning, metadata=metadata, tags=["trade-exit"]
        )
    ...
    def close_all(
        self, parent_trade_id: str, price: float, reasoning: str
    ) -> TradeResult:
        """
    ...
    def bulk_import_trades(
        self, trades: Union[List[Dict[str, Any]], Any]
    ) -> BulkImportResult:
        """
    ...
    def get_open_positions(
        self, ticker: Optional[str] = None, paper: Optional[bool] = None
    ) -> List[Position]:
        """
    ...
    def get_portfolio_summary(self, paper: Optional[bool] = None) -> TradingStats:
        ...
        return TradingStats(
            ...
            profit_factor=float(item.get("profit_factor"))
            if item.get("profit_factor") is not None
            else None,
            total_pnl=float(item.get("total_pnl", 0.0)),
            avg_pnl_percent=float(item.get("avg_pnl_percent"))
            if item.get("avg_pnl_percent") is not None
            else None,
            best_trade_pnl=float(item.get("best_trade_pnl"))
            if item.get("best_trade_pnl") is not None
            else None,
            worst_trade_pnl=float(item.get("worst_trade_pnl"))
            if item.get("worst_trade_pnl") is not None
            else None,
            sharpe_ratio=float(item.get("sharpe_ratio"))
    ...
    def _map_to_bulk_result(self, data: Dict[str, Any]) -> BulkImportResult:
        ...
        errors = [
            BulkImportError(
                index=e.get("index"), ref=e.get("ref"), reason=e.get("reason")
            )
