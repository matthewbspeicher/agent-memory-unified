...
async def upgrade(db: aiosqlite.Connection) -> None:
    ...
    try:
        await db.execute("ALTER TABLE arb_spread_observations ADD COLUMN is_claimed BOOLEAN DEFAULT 0")
        await db.execute("ALTER TABLE arb_spread_observations ADD COLUMN claimed_at TEXT")
        await db.execute("ALTER TABLE arb_spread_observations ADD COLUMN claimed_by TEXT")
    except Exception:
