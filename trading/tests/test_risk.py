...
def test_compute_child_pnl_matches_php():
    ...
    for idx, vector in enumerate(vectors):
        ...
        assert result['pnl'] == vector['expected_net_pnl'], f"Mismatch in PnL for vector {idx} ({vector['description']}): Expected {vector['expected_net_pnl']}, got {result['pnl']}"
        assert result['pnl_percent'] == vector['expected_pnl_percent'], f"Mismatch in PnL percent for vector {idx} ({vector['description']}): Expected {vector['expected_pnl_percent']}, got {result['pnl_percent']}"
