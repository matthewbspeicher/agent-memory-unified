...
class TestFillSimulator:
    def test_market_order_fills_at_close_with_slippage(self):
        sim = FillSimulator(slippage_pct=Decimal("0.001"), fee_per_trade=Decimal("1.00"))
        order = MarketOrder(symbol=_symbol(), side=OrderSide.BUY, quantity=Decimal("10"), account_id="TEST")
    ...
    def test_sell_slippage_goes_down(self):
        ...
        order = MarketOrder(symbol=_symbol(), side=OrderSide.SELL, quantity=Decimal("10"), account_id="TEST")
        fill = sim.simulate(order, bar_close=Decimal("150.00"))
    ...
    def test_limit_buy_fills_when_low_reaches_limit(self):
        ...
        order = LimitOrder(symbol=_symbol(), side=OrderSide.BUY, quantity=Decimal("10"),
                           account_id="TEST", limit_price=Decimal("148.00"))
    ...
    def test_limit_buy_no_fill_above_limit(self):
        sim = FillSimulator(slippage_pct=Decimal("0"), fee_per_trade=Decimal("0"))
        order = LimitOrder(symbol=_symbol(), side=OrderSide.BUY, quantity=Decimal("10"),
                           account_id="TEST", limit_price=Decimal("145.00"))
