...
async def test_get_quote():
    ...
    client = TradierClient(token="test", account_id="acc1", sandbox=True)
    mock_data = {"quotes": {"quote": {"symbol": "AAPL", "last": 150.5, "bid": 150.45, "ask": 150.55}}}
    with patch.object(client, "_request", new_callable=AsyncMock, return_value=mock_data):
...
async def test_place_order():
    ...
    with patch.object(client, "_request", new_callable=AsyncMock, return_value=mock_data):
        result = await client.place_order(
            symbol="AAPL", side="buy", qty=10, order_type="market", duration="day",
        )
