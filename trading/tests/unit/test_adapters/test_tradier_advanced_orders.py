...
from broker.models import (
    BracketOrder, OrderSide, OrderStatus, StopOrder,
...
async def test_stop_order():
    ...
    mock_client.place_order = AsyncMock(return_value={"order": {"id": 123, "status": "ok"}})
    mock_client.get_order = AsyncMock(return_value={
        "id": "123", "status": "filled", "exec_quantity": "10", "avg_fill_price": "149.50",
    })
...
async def test_trailing_stop_amount():
    ...
    mock_client.get_order = AsyncMock(return_value={
        "id": "456", "status": "filled", "exec_quantity": "10", "avg_fill_price": "148.00",
    })
...
async def test_trailing_stop_percent_rejected():
    ...
    assert "trail_amount" in result.message.lower() or "percent" in result.message.lower()
...
async def test_bracket_order_rejected():
    ...
    order = BracketOrder(
        ...
        quantity=Decimal("10"), account_id="acc1",
        take_profit_price=Decimal("170"), stop_loss_price=Decimal("140"),
    )
    result = await om.place_order("acc1", order)

    assert result.status == OrderStatus.REJECTED
    assert "deferred" in result.message.lower() or "not implemented" in result.message.lower()
