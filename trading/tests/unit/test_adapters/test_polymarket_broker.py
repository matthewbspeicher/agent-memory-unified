...
def mock_clob():
    with patch("adapters.polymarket.client.ClobClient") as MockClob:
        ...
        mock_instance.create_or_derive_api_creds.return_value = ApiCreds(
            "key", "secret", "pass"
        )
...
def mock_w3():
    with patch("adapters.polymarket.client.Web3") as MockW3:
        ...
        w3_instance.eth.contract.side_effect = lambda address, abi: (
            usdc_mock if "balanceOf" in str(abi) else ct_mock
        )
...
@pytest.mark.asyncio
async def test_auth_flow_and_idempotent_approval(
    poly_broker, mock_clob, mock_w3, tmp_path
):
    ...
    broker2 = PolymarketBroker(
        client2, PolymarketDataSource(client2), str(creds_path), dry_run=False
    )
