...
async def test_semantic_drift_reembedding(indexer, mock_model):
    ...
    for i in range(5):
        ...
        await indexer._handle_entry_added({"memory_id": f"dummy-{i}", "content": f"dummy {i}", "metadata": {}})
        
    # Add nearby dummy
    mock_model.encode.return_value = nearby_vec
    await indexer._handle_entry_added({"memory_id": "nearby-1", "content": "nearby", "metadata": {}})
