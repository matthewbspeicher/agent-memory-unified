...
async def test_anthropic_provider_generates_and_caches(mock_db, mock_opp_store):
    ...
    mock_llm.complete = AsyncMock(
        return_value=MagicMock(text="RSI signal fired correctly. Entry was well-timed.")
    )

    gen = AutopsyGenerator(
        db=mock_db,
        opp_store=mock_opp_store,
        llm=mock_llm,
    )
...
async def test_openai_compatible_provider(mock_db, mock_opp_store):
    ...
    mock_llm = AsyncMock()
    mock_llm.complete = AsyncMock(
        return_value=MagicMock(text="Trade analysis complete.")
    )
...
async def test_context_includes_all_four_sources(mock_db, mock_opp_store):
    ...
    lessons_cursor.fetchall = AsyncMock(
        return_value=[
