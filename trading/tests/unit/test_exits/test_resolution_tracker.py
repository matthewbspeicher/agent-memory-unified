...
class TestResolutionTrackerKalshi:
    ...
    def tracker(self):
        ...
        paper_store.get_positions = AsyncMock(
            return_value=[_make_position("KXBTCD-25MAR28")]
        )
    ...
    async def test_finalized_yes_calls_resolve(self, tracker):
        tracker._kalshi_client.get_market = AsyncMock(
            return_value={
                "status": "finalized",
                "result": "yes",
            }
...
class TestResolutionTrackerPolymarket:
    ...
    def tracker(self):
        ...
        paper_store = MagicMock()
        paper_store.get_positions = AsyncMock(
            return_value=[_make_position("token-abc123")]
        )
        return ResolutionTracker(
            kalshi_client=None,
    ...
    async def test_resolved_yes_token_calls_yes(self, tracker):
        tracker._poly_client.get_market = MagicMock(
            return_value={
                "closed": True,
                "resolved": True,
