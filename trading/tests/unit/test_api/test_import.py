...
async def test_import_fidelity(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.post("/import/fidelity", files={"file": ("positions.csv", SAMPLE_CSV, "text/csv")})
...
async def test_get_external_portfolio(app):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        await client.post("/import/fidelity", files={"file": ("positions.csv", SAMPLE_CSV, "text/csv")})
