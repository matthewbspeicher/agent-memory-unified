...
def test_get_quote(client, mock_broker):
    mock_broker.market_data.get_quote = AsyncMock(
        return_value=Quote(
...
def test_get_batch_quotes(client, mock_broker):
    mock_broker.market_data.get_quotes = AsyncMock(
        return_value=[
...
def test_get_contract_details(client, mock_broker):
    mock_broker.market_data.get_contract_details = AsyncMock(
        return_value=ContractDetails(
            symbol=Symbol(ticker="AAPL"),
            long_name="Apple Inc.",
            industry="Technology",
            category="Computers",
