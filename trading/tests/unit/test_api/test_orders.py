...
def test_place_market_order(client, mock_broker):
    mock_broker.orders.place_order = AsyncMock(return_value=OrderResult(
        order_id="12345", status=OrderStatus.SUBMITTED,
    ))
...
def test_place_limit_order(client, mock_broker):
    mock_broker.orders.place_order = AsyncMock(return_value=OrderResult(
        order_id="12346", status=OrderStatus.SUBMITTED,
    ))
    resp = client.post("/orders", headers=HEADERS, json={
        "symbol": {"ticker": "AAPL"},
        "side": "BUY",
...
def test_cancel_order(client, mock_broker):
    mock_broker.orders.cancel_order = AsyncMock(return_value=OrderResult(
        order_id="12345", status=OrderStatus.CANCELLED,
    ))
