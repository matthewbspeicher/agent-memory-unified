...
def opp_client(mock_broker):
    ...
    store.list = AsyncMock(return_value=[
        {"id": "opp-1", "agent_name": "rsi", "symbol": "AAPL",
         "signal": "RSI_OVERSOLD", "confidence": 0.85, "reasoning": "Test",
         "status": "pending", "data": "{}", "created_at": "2026-03-25"},
    ])
...
class TestOpportunitiesAPI:
    ...
    def test_get_missing_opportunity(self, opp_client):
        ...
        resp = opp_client.get("/opportunities/missing", headers={"X-API-Key": "test-key"})
        assert resp.status_code == 404

    def test_approve_opportunity(self, opp_client):
        resp = opp_client.post("/opportunities/opp-1/approve-auth", headers={"X-API-Key": "test-key"})
        assert resp.status_code == 200

    def test_reject_opportunity(self, opp_client):
        resp = opp_client.post("/opportunities/opp-1/reject-auth", headers={"X-API-Key": "test-key"})
        assert resp.status_code == 200
