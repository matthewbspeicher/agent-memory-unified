...
from broker.options import build_iron_condor, build_collar, build_protective_put
...
def test_build_iron_condor():
    ...
    # Verify legs
    long_put = [l for l in order.legs if l.side == OrderSide.BUY and l.symbol.right == OptionRight.PUT][0]
    assert long_put.symbol.strike == Decimal("480")
    
    short_put = [l for l in order.legs if l.side == OrderSide.SELL and l.symbol.right == OptionRight.PUT][0]
    assert short_put.symbol.strike == Decimal("490")
    
    short_call = [l for l in order.legs if l.side == OrderSide.SELL and l.symbol.right == OptionRight.CALL][0]
    assert short_call.symbol.strike == Decimal("510")
    
    long_call = [l for l in order.legs if l.side == OrderSide.BUY and l.symbol.right == OptionRight.CALL][0]
