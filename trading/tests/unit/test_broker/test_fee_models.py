...
def test_zero_fee_model():
    model = ZeroFeeModel()
    order = MarketOrder(
        symbol=AAPL, side=OrderSide.BUY, quantity=Decimal("100"), account_id="X"
    )
...
class TestFidelityFeeModel:
    ...
    def test_stock_buy_no_commission(self):
        order = MarketOrder(
            symbol=AAPL, side=OrderSide.BUY, quantity=Decimal("10"), account_id="X"
        )
    ...
    def test_stock_sell_sec_fee(self):
        ...
        order = MarketOrder(
            symbol=AAPL, side=OrderSide.SELL, quantity=Decimal("10"), account_id="X"
        )
        fee = self.model.calculate(order, Decimal("150.00"))
        expected = (Decimal("10") * Decimal("150.00") * Decimal("0.0000278")).quantize(
            Decimal("0.01")
        )
        assert fee == expected
    ...
    def test_stock_sell_sec_fee_minimum(self):
        ...
        order = MarketOrder(
            symbol=AAPL, side=OrderSide.SELL, quantity=Decimal("1"), account_id="X"
        )
    ...
    def test_option_buy_per_contract(self):
        order = MarketOrder(
            symbol=AAPL_OPT, side=OrderSide.BUY, quantity=Decimal("5"), account_id="X"
        )
    ...
    def test_option_sell_per_contract_plus_sec(self):
        order = MarketOrder(
            symbol=AAPL_OPT, side=OrderSide.SELL, quantity=Decimal("2"), account_id="X"
        )
        fill = Decimal("5.00")
...
class TestIBKRFeeModel:
    ...
    def test_stock_buy_normal_order(self):
        ...
        order = MarketOrder(
            symbol=AAPL, side=OrderSide.BUY, quantity=Decimal("200"), account_id="X"
        )
        fee = self.model.calculate(order, Decimal("100.00"))
    ...
    def test_stock_buy_cap_applied(self):
        ...
        order = MarketOrder(
            symbol=AAPL, side=OrderSide.BUY, quantity=Decimal("1000"), account_id="X"
        )
    ...
    def test_stock_sell_includes_finra_taf(self):
        ...
        order = MarketOrder(
            symbol=AAPL, side=OrderSide.SELL, quantity=Decimal("100"), account_id="X"
        )
    ...
    def test_stock_sell_finra_taf_max(self):
        ...
        order = MarketOrder(
            symbol=AAPL, side=OrderSide.SELL, quantity=Decimal("100000"), account_id="X"
        )
    ...
    def test_option_buy_minimum(self):
        ...
        order = MarketOrder(
            symbol=AAPL_OPT, side=OrderSide.BUY, quantity=Decimal("1"), account_id="X"
        )
    ...
    def test_option_buy_above_minimum(self):
        ...
        order = MarketOrder(
            symbol=AAPL_OPT, side=OrderSide.BUY, quantity=Decimal("3"), account_id="X"
        )
