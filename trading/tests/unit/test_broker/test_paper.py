...
from broker.models import Symbol, AssetType, MarketOrder, LimitOrder, OrderSide, Quote, Position, AccountBalance, BrokerCapabilities
...
class MockConnection(BrokerConnection):
    ...
    def on_disconnected(self, callback): pass
    async def reconnect(self): pass


class MockMarketData(MarketDataProvider):
    async def get_quote(self, symbol: Symbol) -> Quote:
        return Quote(symbol=symbol, bid=Decimal("150.0"), ask=Decimal("150.1"), last=Decimal("150.05"))
    async def get_quotes(self, symbols): return []
    async def stream_quotes(self, symbols, callback): pass
    async def get_historical(self, symbol, timeframe, period): return []
    async def get_options_chain(self, symbol, expiry=None): pass
    async def get_contract_details(self, symbol): pass
...
class MockBroker(Broker):
    ...
    @property
    def orders(self): return None
    @property
    def market_data(self): return MockMarketData()
    def capabilities(self): return BrokerCapabilities()
...
async def test_paper_order_raises_on_no_price():
    ...
    market_data.get_quote = AsyncMock(return_value=Quote(
...
class TestPaperBroker:
    ...
    async def test_realized_pnl(self, paper_broker):
        ...
        # Buy 10 @ 150.1
        await paper_broker.orders.place_order("PAPER", MarketOrder(
            symbol=sym, side=OrderSide.BUY, quantity=Decimal("10"), account_id="PAPER"
        ))
        
        # Sell 5 @ 160.0 (simulated via limit order at 160.0)
        await paper_broker.orders.place_order("PAPER", LimitOrder(
            symbol=sym, side=OrderSide.SELL, quantity=Decimal("5"), account_id="PAPER", limit_price=Decimal("160.0")
        ))
