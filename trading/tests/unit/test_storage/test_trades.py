...
class TestTradeStore:
    async def test_save_and_get(self, trade_store):
        await trade_store.save_trade("opp-1", {"order_id": "ORD1", "status": "FILLED"}, {"passed": True})
    ...
    async def test_save_and_get_risk_event(self, trade_store):
        await trade_store.save_risk_event("kill_switch_enabled", {"reason": "daily loss"})
    ...
    async def test_save_trade_with_agent_name(self, trade_store):
        await trade_store.save_trade("opp-1", {"order_id": "ORD1"}, agent_name="rsi_scanner")
        trades = await trade_store.get_trades("opp-1")
        assert len(trades) == 1
