...
def _make_snapshot(agent_name: str, sharpe: float, total_pnl: float = 0.0, win_rate: float = 0.5) -> PerformanceSnapshot:
...
def mock_perf_store():
    store = MagicMock()
    store.get_latest = AsyncMock(side_effect=lambda name: {
...
def mock_runner():
    runner = MagicMock()
    info1 = MagicMock(); info1.name = "rsi_scanner"
    info2 = MagicMock(); info2.name = "volume_spike"
    info3 = MagicMock(); info3.name = "kalshi_news_arb"
...
async def test_sharpe_ranking_sorted_descending(mock_perf_store, mock_runner, mock_db):
    ...
    engine = LeaderboardEngine(perf_store=mock_perf_store, runner=mock_runner, db=mock_db)
...
async def test_elo_update_bounded_swing(mock_perf_store, mock_runner, mock_db):
    ...
    from leaderboard.engine import LeaderboardEngine, MatchResult
    engine = LeaderboardEngine(perf_store=mock_perf_store, runner=mock_runner, db=mock_db)
    rankings = await engine.compute_rankings(profiles=None)
...
async def test_n1_no_matches(mock_db):
    ...
    runner = MagicMock()
    info = MagicMock(); info.name = "only_agent"
...
@pytest.mark.asyncio
async def test_tally_results_updates_win_loss_streak(mock_perf_store, mock_runner, mock_db):
    from leaderboard.engine import LeaderboardEngine
    engine = LeaderboardEngine(perf_store=mock_perf_store, runner=mock_runner, db=mock_db)
    rankings = await engine.compute_rankings(profiles=None)
    matches = engine.run_matches(rankings)
...
@pytest.mark.asyncio
async def test_idempotency_same_snapshot_returns_cached(mock_perf_store, mock_runner, mock_db):
