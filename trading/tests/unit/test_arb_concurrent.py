...
@pytest.mark.asyncio
async def test_arb_concurrent_success(mock_brokers, mock_store, mock_settings, sample_trade):
...
@pytest.mark.asyncio
async def test_arb_concurrent_partial_failure_unwind(mock_brokers, mock_store, mock_settings, sample_trade):
    coordinator = ArbCoordinator(mock_brokers, mock_store, mock_settings)
    ...
    mock_brokers["kalshi"].orders.place_order = AsyncMock(return_value=res_a)
    mock_brokers["polymarket"].orders.place_order = AsyncMock(side_effect=Exception("API Error"))
...
@pytest.mark.asyncio
async def test_arb_concurrent_dual_failure(mock_brokers, mock_store, mock_settings, sample_trade):
    coordinator = ArbCoordinator(mock_brokers, mock_store, mock_settings)
    
    mock_brokers["kalshi"].orders.place_order = AsyncMock(side_effect=Exception("API A Error"))
    mock_brokers["polymarket"].orders.place_order = AsyncMock(side_effect=Exception("API B Error"))
...
@pytest.mark.asyncio
async def test_preflight_slippage_abort(mock_brokers, mock_store, mock_settings, sample_trade):
    coordinator = ArbCoordinator(mock_brokers, mock_store, mock_settings)
