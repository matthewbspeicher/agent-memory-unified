...
from integrations.bittensor.models import MinerRankingInput, RankingWeights, RankingConfig
...
from integrations.bittensor.ranking import compute_rankings, DIRECTION_HEAVY, RETURN_SENSITIVE
...
class TestComputeRankings:
    ...
    def test_single_miner_returns_one_ranking(self):
        inputs = [_make_input()]
        results = compute_rankings(inputs, DIRECTION_HEAVY, _default_config(), _fixed_now())
    ...
    def test_two_miners_different_scores(self):
        inputs = [
            _make_input(hotkey="good", dir_acc=0.9, mag_err=0.01, path_corr=0.8, incentive=0.7),
            _make_input(hotkey="bad", dir_acc=0.3, mag_err=0.10, path_corr=-0.2, incentive=0.3),
        ]
        results = compute_rankings(inputs, DIRECTION_HEAVY, _default_config(), _fixed_now())
    ...
    def test_weight_profiles_are_valid(self):
        assert abs(DIRECTION_HEAVY.direction + DIRECTION_HEAVY.magnitude + DIRECTION_HEAVY.path - 1.0) < 1e-9
        assert abs(RETURN_SENSITIVE.direction + RETURN_SENSITIVE.magnitude + RETURN_SENSITIVE.path - 1.0) < 1e-9
