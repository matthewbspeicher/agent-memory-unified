...
from broker.models import OrderBase, Symbol, AssetType, Quote, MarketOrder
...
@pytest.mark.asyncio
async def test_governor_size_factor(mock_leaderboard, mock_tournament, mock_perf_store, mock_agent_store, mock_settings):
    # Setup
    ...
    mock_tournament.get_all_stages = AsyncMock(return_value={agent_name: 3}) # Stage 3 (Max)
    ...
    trade = MarketOrder(symbol=symbol, side="BUY", quantity=Decimal("100"), account_id="TEST")
...
@pytest.mark.asyncio
async def test_governor_demotion(mock_leaderboard, mock_tournament, mock_perf_store, mock_agent_store, mock_settings):
    governor = CapitalGovernor(mock_leaderboard, mock_tournament, mock_perf_store, mock_agent_store, mock_settings)
    ...
    trade = MarketOrder(symbol=Symbol(ticker="AAPL", asset_type=AssetType.STOCK), side="BUY", quantity=Decimal("100"), account_id="TEST")
    trade.agent_name = agent_name
    result = await governor.evaluate(trade, MagicMock(), PortfolioContext([], MagicMock()))
