...
def _make_config(**overrides):
    defaults = dict(
        name="test_vol", strategy="volume_spike", schedule="cron",
...
def _bars(open_price: str, close_price: str) -> list[Bar]:
    ...
    base = [
        Bar(symbol=sym, open=Decimal("150"), close=Decimal("150"),
            high=Decimal("151"), low=Decimal("149"), volume=1_000_000,
    ...
    base.append(Bar(
        ...
        high=Decimal("155"), low=Decimal("148"),
        volume=1_000_000,
...
async def test_volume_spike_with_positive_price_includes_buy():
    ...
    data.get_quote = AsyncMock(return_value=Quote(
        symbol=sym, last=Decimal("152"), volume=3_000_000, timestamp=datetime.now(),
    ))
...
async def test_volume_spike_with_bearish_bar_no_trade():
    ...
    data.get_quote = AsyncMock(return_value=Quote(
        symbol=sym, last=Decimal("148"), volume=3_000_000, timestamp=datetime.now(),
    ))
