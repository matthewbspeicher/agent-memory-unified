...
async def test_position_monitor_agent_stale_and_underwater():
    config = AgentConfig(
        name="monitor", strategy="position_monitor", schedule="on_demand", action_level=ActionLevel.NOTIFY,
    ...
    p1 = Position(Symbol("AAPL"), quantity=Decimal("10"), avg_cost=Decimal("100"), 
                  market_value=Decimal("900"), unrealized_pnl=Decimal("-100"), realized_pnl=Decimal("0"))
    
    # Position that is not underwater (pnl > 0)
    p2 = Position(Symbol("MSFT"), quantity=Decimal("10"), avg_cost=Decimal("100"), 
                  market_value=Decimal("1100"), unrealized_pnl=Decimal("100"), realized_pnl=Decimal("0"))
    ...
    trades = [
        ...
        {"order_result": json.dumps({"symbol": {"ticker": "AAPL"}}), "created_at": old_date.isoformat()},
        # MSFT traded 10 days ago (not stale)
        {"order_result": json.dumps({"symbol": {"ticker": "MSFT"}}), "created_at": recent_date.isoformat()}
    ]
...
async def test_tax_loss_harvesting_agent():
    config = AgentConfig(
        name="harvester", strategy="tax_loss", schedule="on_demand", action_level=ActionLevel.SUGGEST_TRADE,
    ...
    p1 = Position(Symbol("TSLA"), quantity=Decimal("10"), avg_cost=Decimal("200"), 
                  market_value=Decimal("1400"), unrealized_pnl=Decimal("-600"), realized_pnl=Decimal("0"))
    
    # Position with $800 loss, but traded 10 days ago (wash sale blocked)
    p2 = Position(Symbol("AMZN"), quantity=Decimal("10"), avg_cost=Decimal("200"), 
                  market_value=Decimal("1200"), unrealized_pnl=Decimal("-800"), realized_pnl=Decimal("0"))
    
    # Position with $200 loss (less than 500 minimum)
    p3 = Position(Symbol("GOOGL"), quantity=Decimal("10"), avg_cost=Decimal("200"), 
                  market_value=Decimal("1800"), unrealized_pnl=Decimal("-200"), realized_pnl=Decimal("0"))

    now = datetime.now(timezone.utc)
    trades = [
        {"order_result": {"symbol": {"ticker": "TSLA"}}, "created_at": (now - timedelta(days=40)).isoformat()},
        {"order_result": {"symbol": {"ticker": "AMZN"}}, "created_at": (now - timedelta(days=10)).isoformat()},
        {"order_result": {"symbol": {"ticker": "GOOGL"}}, "created_at": (now - timedelta(days=50)).isoformat()}
    ]
