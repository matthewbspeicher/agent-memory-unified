...
async def test_polymarket_news_arb_agent(mock_app_state):
    ...
    agent.structured_call = AsyncMock(
        return_value={"implied_probability": 70, "confidence": 90, "reasoning": "sure"}
    )
...
async def test_cross_platform_arb_agent(mock_kalshi_ds, mock_poly_ds):
    ...
    agent = CrossPlatformArbAgent(
        config=config, kalshi_ds=mock_kalshi_ds, polymarket_ds=mock_poly_ds
    )
