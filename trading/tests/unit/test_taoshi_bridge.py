...
def _write_position(base_dir: Path, hotkey: str, symbol: str, uuid: str, position: dict) -> Path:
    """Write a position JSON file to the expected directory structure."""
    pos_dir = base_dir / "validation" / "miners" / hotkey / "positions" / symbol / "open"
...
def _remove_position(base_dir: Path, hotkey: str, symbol: str, uuid: str) -> None:
    ...
    path = base_dir / "validation" / "miners" / hotkey / "positions" / symbol / "open" / f"{uuid}.json"
    if path.exists():
        path.unlink()
...
class TestHashPosition:
    ...
    def test_different_order_count_different_hash(self):
        pos1 = _make_position("abc", orders=[
            {"order_type": "LONG", "leverage": 1.0, "price": 50000, "processed_ms": 100},
        ])
        pos2 = _make_position("abc", orders=[
            {"order_type": "LONG", "leverage": 1.0, "price": 50000, "processed_ms": 100},
            {"order_type": "LONG", "leverage": 2.0, "price": 51000, "processed_ms": 200},
        ])
    ...
    def test_different_leverage_different_hash(self):
        ...
        pos2 = _make_position("abc", orders=[
            {"order_type": "LONG", "leverage": 5.0, "price": 50000, "processed_ms": 100},
        ])
        assert TaoshiBridge._hash_position(pos1) != TaoshiBridge._hash_position(pos2)
    ...
    def test_different_timestamp_different_hash(self):
        ...
        pos2 = _make_position("abc", orders=[
            {"order_type": "LONG", "leverage": 1.0, "price": 50000, "processed_ms": 999},
        ])
        assert TaoshiBridge._hash_position(pos1) != TaoshiBridge._hash_position(pos2)
