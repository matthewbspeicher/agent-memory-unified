...
async def test_regime_trending_bull_with_memories():
    ...
    with patch.object(provider, "_fetch_bars", new_callable=AsyncMock, return_value=bars):
        report = await provider.analyze("BTCUSD")
...
async def test_regime_fetch_bars_failure():
    ...
    with patch.object(provider, "_fetch_bars", new_callable=AsyncMock, side_effect=Exception("no data")):
        report = await provider.analyze("BTCUSD")
