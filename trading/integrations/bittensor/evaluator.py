...
class MinerEvaluator:
    ...
    async def evaluate_pending_windows(self, now: datetime | None = None) -> int:
        ...
        for window in windows:
            try:
                ...
                logger.error("Failed to evaluate window %s: %s", window.window_id, e, exc_info=True)
    ...
    async def _evaluate_window(self, window: BittensorEvaluationWindow):
        ...
        symbol = Symbol(ticker=window.symbol, asset_type=AssetType.FOREX) # Most BT signals are FX
        ...
        if not bars:
            logger.warning("No bar data for %s, skipping window %s", window.symbol, window.window_id)
            return
        ...
        if len(window_bars) < window.prediction_size:
            logger.debug("Not enough bars yet for window %s (%d/%d)", 
                         window.window_id, len(window_bars), window.prediction_size)
            return
        ...
        if accuracy_records:
            ...
            logger.info("Evaluated %d forecasts for window %s", len(accuracy_records), window.window_id)
    ...
    def _score_forecast(
        ...
        predicted_return = (predicted_path[-1] - predicted_path[0]) / predicted_path[0] if predicted_path else 0.0
    ...
    async def refresh_rankings(self):
        ...
        async with self.store._db.execute("SELECT DISTINCT miner_hotkey FROM bittensor_accuracy_records") as cursor:
