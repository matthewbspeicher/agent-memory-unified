...
from api.deps import set_broker, set_agent_runner, set_risk_engine, set_opportunity_store, set_trade_store, set_event_bus
...
class ServiceContainer:
    ...
    def _resolve_path(self, name: str) -> str:
        p_str = f"/app/{name}"
        if os.path.isfile(p_str): return p_str
        p_str = name
        if os.path.isfile(p_str): return p_str
        p_str = str(self.app_root / name)
        if os.path.isfile(p_str): return p_str
    ...
    async def bootstrap(self):
        ...
        if self.settings.redis_url:
            try:
                ...
                self.redis_bridge = RedisSignalBridge(self.event_bus, self.settings.redis_url, node_id)
    ...
    async def _load_learning_config(self):
        ...
        if os.path.isfile(l_path):
            ...
            self.learning_config = LearningConfig(memory={"enabled": False}, strategy_health={"enabled": False})
    ...
    async def _init_multi_brokers(self):
        ...
        if self.settings.paper_trading:
            ...
            await self.db.execute("INSERT OR REPLACE INTO paper_accounts (account_id, net_liquidation, buying_power, cash, maintenance_margin) VALUES ('PAPER', ?, ?, ?, 0.0)", (_initial, _initial, _initial))
    ...
    async def _init_data_bus(self):
        ...
        if self.settings.massive_key:
            try:
                ...
                _sources = [MassiveDataSource(MassiveClient(api_key=self.settings.massive_key)), *sources]
                sources = _sources
            except Exception as e: logger.warning(f"Massive setup failed: {e}")

        _acc_id = "PAPER" if self.settings.paper_trading else ""
        self.data_bus = DataBus(sources=sources, broker=self.broker, trade_store=self.trade_store, account_id=_acc_id)
        self.data_bus._llm_client = self.llm_client
        
        if self.settings.paper_trading and hasattr(self.broker, "_market_data_provider"):
    ...
    async def _init_intelligence_system(self):
        ...
        # Arena Sync
        reg_token = self.settings.remembr_owner_token or self.settings.remembr_agent_token
        if reg_token:
            self.remembr_sync = RemembrArenaSync(token=reg_token, db=self.db, base_url=self.settings.remembr_base_url)
            
        # Journal
        token = self.settings.remembr_private_token
        if token:
            _client = AsyncRemembrClient(agent_token=token, base_url=self.settings.remembr_base_url)
            if self.settings.journal_index_enabled:
                from journal.indexer import JournalIndexer
                self.journal_indexer = JournalIndexer(self.event_bus, _client, gpu_enabled=self.settings.gpu_enabled)
                await self.journal_indexer.start()
                
            self.journal_manager = JournalManager(client=_client, event_bus=self.event_bus, indexer=self.journal_indexer, oracle_url=self.settings.oracle_url)
    ...
    async def _init_execution_pipeline(self):
        ...
        self.services["slippage_loop"] = SlippageFeedbackLoop(tracker=self.services["exec_tracker"], perf_store=self.perf_store, agent_store=self.agent_store)
        
        self.shadow_executor = ShadowExecutor(store=self.shadow_store, data_bus=self.data_bus)
        self.shadow_outcome_resolver = ShadowOutcomeResolver(store=self.shadow_store, data_bus=self.data_bus)
        self.tasks.append(asyncio.create_task(self._shadow_resolver_loop()))
        
        self.services["sf_capture"] = SignalFeatureCapture(store=self.sf_store, data_bus=self.data_bus)

    async def _shadow_resolver_loop(self):
        while True:
            try: await self.shadow_outcome_resolver.resolve_due(limit=25)
            except Exception as e: logger.warning(f"Shadow resolver failed: {e}")
    ...
    async def _init_agent_framework(self):
        ...
        if self.settings.slack_webhook_url:
            _notifiers.append(SlackNotifier(self.settings.slack_webhook_url, _api_base, api_key=self.settings.api_key))
        self.services["notifier"] = CompositeNotifier(_notifiers)
        
        # Health Engine
        _sh_store = StrategyHealthStore(self.db)
        _sh_cfg = StrategyHealthConfig.from_learning_config(getattr(self.learning_config, "strategy_health", None))
        health_engine = StrategyHealthEngine(health_store=_sh_store, perf_store=self.perf_store, config=_sh_cfg)
        ...
        # Load agents
        _path = self._resolve_path(self.settings.agents_config or ("agents.paper.yaml" if self.settings.paper_trading else "agents.yaml"))
        agents = load_agents_config(_path, prompt_store=SqlPromptStore(self.db))
        for a in agents: self.runner.register(a)
    ...
    async def shutdown(self):
        ...
        if self.runner: await self.runner.stop_polling()
        if self.journal_indexer: await self.journal_indexer.stop()
        if self.redis_bridge: await self.redis_bridge.stop()
