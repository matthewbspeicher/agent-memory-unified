...
async def execute_arbitrage(req: ExecuteArbRequest, request: Request):
    ...
    if not spread_store or not arb_coordinator:
        raise HTTPException(status_code=501, detail="Arbitrage executioner not configured")
...
async def get_governor_status(request: Request):
    ...
    for agent_name, ranking in gov._cache.rankings.items():
        ...
        size_factor = max(0.1, min(1.0, sharpe_mult * stage_mult * gov._gov_config.scaling_factor))
        ...
        results.append({
            ...
            "max_allocation_usd": round(gov._gov_config.base_allocation_usd * size_factor, 2)
        })
