...
class HistoricalDataSource(DataSource):
    ...
    async def get_quote(self, symbol: Symbol) -> Quote:
        ...
        if not valid_bars:
            raise ValueError(
                f"No historical data for {symbol.ticker} at {self.current_time}"
            )
...
class ReplayDataBus(DataBus):
    ...
    def __init__(
        ...
        starting_balance: Decimal = Decimal("100000.0"),
    ) -> None:
...
class BacktestEngine:
    ...
    async def simulate_execution(
        self, opportunities: list[Opportunity], t: datetime
    ) -> list[Opportunity]:
        executed = []
...
class FillSimulator:
    ...
    def _fill_limit(
        ...
        if order.side == OrderSide.BUY:
            ...
            if low <= limit:
                return SimulatedFill(
                    price=limit, quantity=order.quantity, fees=self.fee_per_trade
                )
            return None
        else:
