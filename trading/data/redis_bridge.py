...
class RedisSignalBridge:
    ...
    def __init__(
        self, event_bus: EventBus, redis_url: str, node_id: Optional[str] = None
    ):
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info(
                f"RedisSignalBridge started on node {self._node_id} (channel: {self._channel})"
            )
            ...
            async for event in self._event_bus.subscribe():
                ...
                if topic in ("agent_signal", "opportunity"):
                    ...
                    if (
                        ...
                        and data.get("_remote_node") == self._node_id
                    ):
                        continue
            ...
            async for message in self._pubsub.listen():
                ...
                if message["type"] == "message":
                    try:
                        ...
                        logger.warning(
                            f"Received malformed JSON from Redis: {message['data']}"
                        )
                    except Exception as e:
