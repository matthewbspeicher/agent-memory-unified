...
from broker.models import Quote, Symbol, Bar, OptionsChain
...
class ExchangeDataSource(DataSource):
    ...
    async def get_order_book(self, symbol: Symbol, limit: int = 20) -> dict[str, Any]:
        """Fetch real-time order book for slippage estimation."""
        ticker_map = {"BTCUSD": "BTC/USDT", "ETHUSD": "ETH/USDT", "SOLUSD": "SOL/USDT"}
        ccxt_symbol = ticker_map.get(
            symbol.ticker, symbol.ticker.replace("USD", "/USDT")
        )

        try:
            exchange = next(iter(self._client._exchanges.values()), None)
            if exchange is None:
                return {"bids": [], "asks": []}
            return await exchange.fetch_order_book(ccxt_symbol, limit=limit)
        except Exception as e:
            logger.warning(
                f"ExchangeDataSource failed to fetch order book for {symbol.ticker}: {e}"
            )
            return {"bids": [], "asks": []}
    ...
    async def get_historical(
        ...
        raise NotImplementedError("ExchangeDataSource does not support historical bars")

    async def get_options_chain(self, symbol: Symbol) -> OptionsChain:
        raise NotImplementedError("ExchangeDataSource does not support options chains")
