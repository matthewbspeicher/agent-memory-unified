...
from broker.models import Bar, OptionsChain, Quote, Symbol, AssetType
...
class BrokerHistoricalSource(DataSource):
    ...
    async def get_historical(
        ...
        def _fetch():
            ...
            logger.info(
                f"Fetching historical data for {ticker} (original: {symbol.ticker})"
            )
    ...
    async def get_quote(self, symbol: Symbol) -> Quote:
        raise NotImplementedError(
            "Use YahooFinanceSource or BrokerSource for live quotes"
        )

    async def get_options_chain(self, symbol: Symbol) -> OptionsChain:
        raise NotImplementedError(
            "Options historical data ingestion not yet implemented"
        )
