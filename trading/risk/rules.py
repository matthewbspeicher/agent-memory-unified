...
class PortfolioContext:
    ...
    price_histories: dict[str, list[Decimal]] = field(default_factory=dict) # ticker -> price series
...
class MaxPositionSize(RiskRule):
    ...
    def evaluate(self, trade: OrderBase, quote: Quote, ctx: PortfolioContext) -> RiskResult:
        ...
        if projected_qty > self.max_shares:
            return RiskResult(passed=False, rule_name=self.name,
...
class MaxPortfolioExposure(RiskRule):
    ...
    def evaluate(self, trade: OrderBase, quote: Quote, ctx: PortfolioContext) -> RiskResult:
        ...
        external_value = sum(
            Decimal(str(b.get("net_liquidation", 0)))
            for b in ctx.external_balances
        )
        total_portfolio = ibkr_net_liq + external_value
        if total_portfolio == 0:
            return RiskResult(passed=False, rule_name=self.name, reason="Portfolio value is zero")
...
class MaxComboDelta(RiskRule):
    ...
    def evaluate(self, trade: OrderBase, quote: Quote, ctx: PortfolioContext) -> RiskResult:
        ...
        if isinstance(trade, ComboOrder):
            current_price = quote.last if quote.last else (quote.ask or quote.bid or Decimal("0"))
            if current_price == 0:
                return RiskResult(passed=False, rule_name=self.name, reason="Cannot estimate Delta: no underlying price")
            
            for leg in trade.legs:
                if not leg.symbol.strike or not leg.symbol.right or not leg.symbol.expiry:
                    return RiskResult(passed=False, rule_name=self.name, reason="Missing option details for Delta estimation")
                
                leg_delta = estimate_option_delta(current_price, leg.symbol.strike, leg.symbol.right, leg.symbol.expiry)
                ...
                multiplier = Decimal(str(leg.symbol.multiplier)) if leg.symbol.multiplier else Decimal("100")
                position_delta = leg_delta * Decimal(str(leg.ratio)) * trade.quantity * multiplier
...
class MaxDrawdownPct(RiskRule):
    ...
    def evaluate(self, trade: OrderBase, quote: Quote, ctx: PortfolioContext) -> RiskResult:
        ...
        drawdown_pct = ((self.high_water_mark - portfolio_value) / self.high_water_mark) * Decimal("100")
