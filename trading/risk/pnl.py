...
def compute_child_pnl(child_entry: str | Decimal, parent_entry: str | Decimal, 
                      child_qty: str | Decimal, parent_qty: str | Decimal, 
                      parent_direction: str, parent_fees: str | Decimal, 
                      child_fees: str | Decimal) -> dict:
    """
    ...
    if cost_basis > Decimal('0'):
        ...
        pnl_percent = (ratio * Decimal('100')).quantize(FOUR_PLACES, rounding=ROUND_DOWN)
    else:
        pnl_percent = FOUR_PLACES
        
    return {
        'pnl': f"{net_pnl:0.8f}",
        'pnl_percent': f"{pnl_percent:0.4f}"
    }
